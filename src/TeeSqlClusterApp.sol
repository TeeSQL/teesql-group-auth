    ...
    IKmsRootRegistry
{
    ...
    // --- Onboarding ---
    struct OnboardMsg { bytes32 fromMember; bytes encryptedPayload; }
    mapping(bytes32 => OnboardMsg[]) internal _onboarding;

    // --- Signer authorization (cluster-scoped) ---
    struct AuthorizedSigner { uint8 permissions; bool active; uint256 authorizedAt; }
    mapping(address => AuthorizedSigner) public authorizedSigners;

    // --- Leader lease ---
    struct Lease { bytes32 memberId; uint256 epoch; uint256 expiresAt; }
    ...
    event MemberRegistered(bytes32 indexed memberId, address indexed instanceId, address indexed passthrough, string role);
    ...
    function initialize(
        ...
        if (_owner == address(0) || _pauser == address(0) || _kms == address(0)) revert ZeroAddress();
    ...
    function createMember(bytes32 salt) external whenNotPaused returns (address passthrough) {
        bytes32 effectiveSalt = salt == bytes32(0)
            ? bytes32(uint256(nextMemberSeq++))
            : salt;
    ...
    function predictMember(bytes32 salt) external view returns (address) {
        ...
        bytes32 bytecodeHash = keccak256(abi.encodePacked(
            type(TeeSqlClusterMember).creationCode,
            abi.encode(address(this))
        ));
        return address(uint160(uint256(keccak256(abi.encodePacked(
            bytes1(0xff), address(this), effectiveSalt, bytecodeHash
        )))));
    }
    ...
    function _verifyCall(CallAuth calldata a, bytes4 selector, bytes memory args)
        internal returns (bytes32)
    {
        ...
        unchecked { memberNonce[a.memberId] = a.nonce + 1; }
        return a.memberId;
    }
    ...
    function claimLeader(CallAuth calldata auth, bytes calldata newEndpoint) external whenNotPaused {
        ...
        leaderLease = Lease({
            memberId: memberId,
            epoch: newEpoch,
            expiresAt: block.timestamp + leaseTTL
        });
    ...
    function addComposeHash(bytes32 h) external override onlyOwner { allowedComposeHashes[h] = true;  emit ComposeHashAdded(h); }
    function removeComposeHash(bytes32 h) external override onlyOwner { allowedComposeHashes[h] = false; emit ComposeHashRemoved(h); }
    function addDevice(bytes32 d) external override onlyOwner { allowedDeviceIds[d] = true;  emit DeviceAdded(d); }
    function removeDevice(bytes32 d) external override onlyOwner { allowedDeviceIds[d] = false; emit DeviceRemoved(d); }
    function setAllowAnyDevice(bool v) external onlyOwner { allowAnyDevice = v; emit AllowAnyDeviceSet(v); }
    ...
    function removeKmsRoot(address r) external onlyOwner { allowedKmsRoots[r] = false; emit KmsRootRemoved(r); }
    ...
    function revokeSigner(address s) external onlyOwner { authorizedSigners[s].active = false; emit SignerRevoked(s); }

    function setPeerApp(address p, bool allowed) external onlyOwner { allowedPeerApps[p] = allowed; emit PeerAppSet(p, allowed); }
    function setLeaseTTL(uint256 t) external onlyOwner { leaseTTL = t; emit LeaseTTLSet(t); }
    ...
    function getMember(bytes32 id) external view returns (Member memory) { return _members[id]; }
    function getOnboarding(bytes32 id) external view returns (OnboardMsg[] memory) { return _onboarding[id]; }

    function supportsInterface(bytes4 id)
        public view virtual override(AccessControlUpgradeable, IERC165)
        returns (bool)
    {
